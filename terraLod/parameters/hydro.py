...
hydrology_params = {
    ...
    'infiltration_capacity':  0.35,   # fraction [0,1] of precipitation absorbed by soil
                                       # 0.05 was far too low: ~90% of rain became runoff, rivers overwhelmingly strong
    'land_evap_fraction':     0.40,   # max PET fraction [0,1] at 30 °C on land
                                       # 0.05 was too low: combined loss was only ~10%; now ~61% → realistic runoff
    'lake_open_evap_mm':      900.0,  # open-water potential evaporation (mm/year) — lower → more overflow
    'spill_erosion_depth':    0.003, # normalised height eroded at lake spill point per iteration
                                       # 0.01 = 30 m/iter × 16 iters = up to 480 m total — saddles collapsed instantly
                                       # 0.003 = ~9 m/iter, much more geologically realistic
    'max_overflow_iterations': 10,    # cascade depth cap — more iterations → longer river chains
    'slope_exp':               1.8,   # MFD exponent: 1=dispersed, 2.5=channelised D8-like rivers
