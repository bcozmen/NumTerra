    ...
    for i in prange(rows):
        for j in range(cols):
            if i == 0:
                ...
                i0 = rows - 2; i1 = rows - 1; sx = 1.0 / dx
            else:
                i0 = i - 1; i1 = i + 1; sx = 0.5 / dx
            if j == 0:
                j0 = 0; j1 = 1; sy = 1.0 / dy
            elif j == cols - 1:
                j0 = cols - 2; j1 = cols - 1; sy = 1.0 / dy
            else:
                j0 = j - 1; j1 = j + 1; sy = 0.5 / dy
            vx = v_x[i, j]; vy = v_y[i, j]
            out0[i,j] = f0[i,j] - (vx*(f0[i1,j]-f0[i0,j])*sx + vy*(f0[i,j1]-f0[i,j0])*sy)*dt
            out1[i,j] = f1[i,j] - (vx*(f1[i1,j]-f1[i0,j])*sx + vy*(f1[i,j1]-f1[i,j0])*sy)*dt
            out2[i,j] = f2[i,j] - (vx*(f2[i1,j]-f2[i0,j])*sx + vy*(f2[i,j1]-f2[i,j0])*sy)*dt
...
def semi_lagrangian_advect(f0, f1, f2, v_x, v_y, dx, dy, dt):
    ...
    for i in prange(rows):
        for j in range(cols):
            ...
            src_i = i - v_x[i, j] * dt / dx
            src_j = j - v_y[i, j] * dt / dy
...
def wind_accelerate(P, v_x, v_y, dx, dy, rho_air, f, wind_friction, dt):
    ...
    for i in prange(rows):
        for j in range(cols):
            if i == 0:          dP_dx = (P[1, j]       - P[0, j])       / dx
            elif i == rows - 1: dP_dx = (P[rows-1, j]  - P[rows-2, j])  / dx
            else:               dP_dx = (P[i+1, j]     - P[i-1, j])     / (2*dx)
            if j == 0:          dP_dy = (P[i, 1]       - P[i, 0])       / dy
            elif j == cols - 1: dP_dy = (P[i, cols-1]  - P[i, cols-2])  / dy
            else:               dP_dy = (P[i, j+1]     - P[i, j-1])     / (2*dy)
            vx = v_x[i, j]; vy = v_y[i, j]
...
def pressure_project(v_x, v_y, dx, dy, iterations):
    ...
    for _ in range(iterations):
        for i in prange(rows):
            for j in range(cols):
                p_l = p[i-1, j] if i > 0      else p[0,      j]
                p_r = p[i+1, j] if i < rows-1 else p[rows-1, j]
                p_u = p[i, j-1] if j > 0      else p[i,      0]
                p_d = p[i, j+1] if j < cols-1 else p[i, cols-1]
    ...
    for i in prange(rows):
        for j in range(cols):
            if i == 0:
                ...
                h_ip = max(H[1,      j] - sea_level, 0.0); sx = 1.0 / dx
            elif i == rows - 1:
                h_im = max(H[rows-2, j] - sea_level, 0.0)
                h_ip = max(H[rows-1, j] - sea_level, 0.0); sx = 1.0 / dx
            else:
                h_im = max(H[i-1, j] - sea_level, 0.0)
                h_ip = max(H[i+1, j] - sea_level, 0.0); sx = 0.5 / dx
            if j == 0:
                h_jm = max(H[i, 0]      - sea_level, 0.0)
                h_jp = max(H[i, 1]      - sea_level, 0.0); sy = 1.0 / dy
            elif j == cols - 1:
                h_jm = max(H[i, cols-2] - sea_level, 0.0)
                h_jp = max(H[i, cols-1] - sea_level, 0.0); sy = 1.0 / dy
            else:
                h_jm = max(H[i, j-1] - sea_level, 0.0)
                h_jp = max(H[i, j+1] - sea_level, 0.0); sy = 0.5 / dy
            w = v_x[i, j] * (h_ip - h_im) * sx + v_y[i, j] * (h_jp - h_jm) * sy
