...
from utils import get_interpolator
...
@dataclass
class HydroParams:
    sea_level_percentile : float = 0.25
    init_lake_area_threshold : float = 10.0
    river_threshold : float = 10.0
    min_lake_river_acc : float = 10000.0
    infiltration_capacity : float = 0.55
    land_evap_fraction : float = 0.60
    lake_open_evap_mm : float = 2000.0
    spill_erosion_depth : float = 0.01
    max_overflow_iterations : int = 40
    slope_exp : float = 2.5
...
class Hydrology:
    def __init__(self, height_map, world_config, config: HydroParams = HydroParams()):
        self.height_map = height_map
        self.world_config = WorldConfig(**world_config) if isinstance(world_config, dict) else world_config
        self.config = config
    ...
    def generate(self, height_map, pts):
        sea_mask = self.get_sea_mask(height_map, pts)
        hydro_maps = {
            "sea_mask": sea_mask,
            "sea_level": self.maps['sea_level'],
        }
        return hydro_maps

    def scale_hydro_params(self):
        dx_m = self.world_config.max_size / (self.world_config.shape[0] - 1)
        dy_m = self.world_config.max_size / (self.world_config.shape[1] - 1)
        self.config.init_lake_area_threshold = max(1, int(self.config.init_lake_area_threshold * 1e6 / (dx_m * dy_m)))

    # --- Sea ---
    def init_sea(self):
        sea_level = np.percentile(self.height_map, self.config.sea_level_percentile * 100)
        sea_mask = detect_sea(self.height_map, sea_level)
        sea_interpolator = get_interpolator(sea_mask.astype(float))
        self.maps['sea_mask'] = sea_mask
        self.maps['sea_level'] = sea_level
        self.maps['sea_interp'] = sea_interpolator
...
class HydrologyOld:
    ...
    def init_sea(self):
        ...
        sea_mask = detect_sea(self.height_map, sea_level)
        sea_interpolator = self.get_interpolators(sea_mask.astype(float))
        return sea_mask, sea_level, sea_interpolator
