...
__all__ = ["normalize", "get_cell_size", "get_grid", "get_grad", "get_lat_grid", "get_normalized_distance_to_mask"]
...
def get_grad(height_map, cell_sizes = (1, 1), max_altitude=1.0):
    height_map = height_map * max_altitude  # Scale to actual altitude range


    grad_i, grad_j = np.gradient(height_map, *cell_sizes)
    grad_mag = np.sqrt(grad_i**2 + grad_j**2)   # radians
    return grad_mag, grad_i, grad_j
